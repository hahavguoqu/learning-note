...
- 从根节点开始，比较要插入的值。
- 如果小于当前节点，则走向左子树；如果大于，则走向右子树。
- 直到找到一个 `nullptr` 的位置，这就是新节点的插入点。

**构建（Build）：**

- 常规：将一个一个节点插入即可、
- 特殊：假如给出的初始列表已经排好序，那么二叉搜索树会退化成链表。此时采用方法保持平衡，例如：
...
Node* sortedArrayToBST(const std::vector<int>& nums, int start, int end) {
        ...
        // 3. 递归构建左右子树(这里默认数组递增排序)
        node->left = sortedArrayToBST(nums, start, mid - 1); // 左半部分
        node->right = sortedArrayToBST(nums, mid + 1, end); // 右半部分
...
- 和插入的路径逻辑一样，直到找到目标值（返回true或节点指针）或遇到 `nullptr`（返回false或nullptr）。

**查找最大/最小值 (Find Min / Max)：**

- 从根节点开始，找最小值就一直往左走 (`left`)，直到尽头。
- 找最大值就一直往右走 (`right`)，直到尽头。

**删除 (Delete)：**

这是最复杂的操作，需要分三种情况讨论：
1. **要删除的节点是叶子节点（没有孩子）：** 直接删除，将其父节点的指针设为 `nullptr`。
2. **要删除的节点只有一个孩子：** 将其父节点的指针指向它的*那个*孩子。
3. **要删除的节点有两个孩子：** 这是关键。你需要找到一个“替代者”来填补它的位置。通常有两种选择：
   - 其左子树中的**最大值**（即“前驱”）。
   - 其右子树中的**最小值**（即“后继”）。
   - 用替代者的值覆盖要删除的节点的值，然后*递归地删除那个替代者节点*（替代者最多只有一个孩子，所以会转换成情况1或2）。
